...
const dashboardConvoysList = document.getElementById('dashboardConvoysList');
...
export async function loadConvoys() {
  ...
  try {
    ...
    if (dashboardConvoysList && isDashboardPage()) {
      dashboardConvoysList.innerHTML = '';

      if (convoys.length === 0) {
        dashboardConvoysList.innerHTML = '<p>You are not in any active convoys yet.</p>';
        return;
      }

      convoys.forEach(function (convoy) {
        const div = document.createElement('div');
        div.className = 'dashboard-convoy-row';
        div.innerHTML = `
          <span>${convoy.name}</span>
          <small>Join Code: ${convoy.joinCode} • ${convoy.role}</small>
        `;
        dashboardConvoysList.appendChild(div);
      });
